...
let Price = document.querySelector(".Price");
let talons = document.getElementById("talon");
let basket = document.getElementById("basket");
// Get category data
const querySnapshot = await getDocs(collection(db, "category"));
let tabCategory = [];
querySnapshot.forEach((doc) => {
  tabCategory.push(doc.data());
});
for (let i = 0; i < tabCategory.length; i++) {
  talons.innerHTML = tabCategory[i].categoryName;
  basket.innerHTML = tabCategory[i].categoryName;
}
...
let getEventail = tabEventail.map((e) => {
  ...
  <div class="col-lg-3">
    ...
    <p class="text-secondary text-center">${e.title}</p>
    <p class="text-center">${e.title2}</p>
    <p class="text-center">${e.prix} FCFA</p>
  </div>
  `;
});
...
let getBS = tabBestSellers.map((b) => {
  ...
  <div class="col-lg-3 p-0">
    ...
    <p class="text-secondary text-center">${b.title}</p>
    <p class="text-center">${b.title2}</p>
    <p class="text-center">${b.prix} FCFA</p>
  </div>
  `;
});
...
<p class="text-secondary text-center">${o.title}</p>
<p class="text-center">${o.title2}</p>
<p class="text-center">${o.prix} FCFA</p>
</div>
  `;
});
// // LOGIC EVENTAIL
...
// let p = 0;
for (let i = 0; i < tabEventail.length; i++) {
  for (let i = 0; i < imgEventail.length; i++) {
...
for (let i = 0; i < tabBestSellers.length; i++) {
  for (let i = 0; i < imgBestSellers.length; i++) {
    ...
    imgBestSellers[i].addEventListener("click", function () {
      ...
      let incr = p++;
      panier.innerHTML = incr;
      teste.innerHTML = `<img src="images/${test.img}" style="width:80px">`;
      product.innerHTML = test.title;
      quantity.innerHTML = `<p class="text-secondary">Quantity: ${incr}</p>`;
      price.innerHTML = `<p>${test.prix}FCFA</p>`;
      let prixFinal = test.prix * incr;
...
for (let i = 0; i < tabOthers.length; i++) {
